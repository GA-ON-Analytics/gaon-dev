...
## ★ 2026-08-04 — 동기화가 스크립트 한 줄로 끝난다

```bash
python -m scripts.sync_from_ml --check   # 무엇이 다른지만 본다
python -m scripts.sync_from_ml           # 복사 + 지도 개요 생성 + 검증
```

아래 1·2·3번은 **ML 파이프라인 출력에 이미 들어와 있음을 확인했다.** 재생성해도 사라지지 않는다.
로컬 후처리는 더 이상 필요 없다.

| 항목 | 확인 결과 (종로구 표본) |
|---|---|
| ① 모델 피처 8개 | 100m 출력에 **8/8 존재** |
| ② `member_grid_ids` | 250m 447건 중 **빈 값 0건** |
| ③ 최근접 쉼터 name/addr/lon/lat | 250m **447/447건 채워짐** |

**대신 새 문제가 하나 드러났다.** 지도 개요 파일 `seoul_grid_100m_map.geojson`(40MB)을
**ML 파이프라인이 만들지 않는다.** 누가 손으로 만든 뒤 재생성 경로가 사라져서, ML을 다시 돌려도
개요 색칠만 옛 데이터로 남아 있었다(`detail-panel-progress.md`에 "재생성 경로 불명"으로 기록).

→ `scripts/sync_from_ml.py`가 ML의 `seoul_grid_100m.geojson`(속성 53개, 121MB)에서
지도가 쓰는 **속성 13개만** 남겨 생성한다. 좌표는 손대지 않는다.
지도용 속성이 하나라도 빠지면 **조용히 넘어가지 않고 멈춘다** — 색이 사라진 채 배포되는 걸 막는다.

**아직 남은 것**: `member_grid_ids`가 **문자열**로 저장된다(`str(ndarray)`).
`split_dashboard_by_gu.py`의 `read_file()`→`to_file()` 왕복에서 리스트가 문자열이 되는 문제로,
앱이 임시 파싱으로 대응 중이다. 근본 해결은 ML 레포 쪽이다(이슈 #35 보류 #2).
